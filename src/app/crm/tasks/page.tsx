...
import { CrmTask, CrmTaskStatus, CrmTaskType } from "@/types/crm";
...
export default function CrmTasksPage() {
  ...
  const [typeFilter, setTypeFilter] = useState<CrmTaskType | "all">("all");
  ...
  const getTypeLabel = (type: CrmTaskType) => {
    switch (type) {
      case "general": return "Allgemein";
      case "interview": return "Interview";
      case "follow_up": return "Follow-up";
      case "client_call": return "Kunden-Call";
  ...
  const handleCreateTask = (values: TaskFormValues) => {
    ...
    const newTask: CrmTask = {
      ...
      type: values.type,
      status: "open", // neue Aufgaben starten als „Offen“
  ...
  }, [tasks, searchTerm, statusFilter, typeFilter, dueFilter]);
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Organisiere deinen Recruiting-Alltag mit Aufgaben für Kandidaten, Jobs und Firmen.
          </p>
        </div>
        <button
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <div className="relative">
              <select
                className="block w-full rounded-md border-0 py-1.5 pl-3 pr-8 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6"
                value={typeFilter}
                onChange={(e) => setTypeFilter(e.target.value as CrmTaskType | "all")}
              >
            ...
            <div className="relative">
              <select
                className="block w-full rounded-md border-0 py-1.5 pl-3 pr-8 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6"
                value={dueFilter}
                onChange={(e) => setDueFilter(e.target.value as "all" | "today" | "this_week" | "later")}
              >
      ...
      <div className="hidden md:block rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50 text-gray-500">
            <tr>
              <th scope="col" className="px-6 py-3 text-left font-medium uppercase tracking-wider">Aufgabe</th>
              <th scope="col" className="px-6 py-3 text-left font-medium uppercase tracking-wider">Typ</th>
            ...
            {filteredTasks.length === 0 && (
              <tr>
                <td colSpan={7} className="px-6 py-8 text-center text-gray-500">
                  Keine Aufgaben gefunden. Filter anpassen oder Suchbegriff ändern.
                </td>
              </tr>
            )}
          </tbody>
